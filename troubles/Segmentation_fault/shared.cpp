...
sg4::MutexPtr SharedDatabase::oclient_mutex = nullptr;
sg4::MutexPtr SharedDatabase::dclient_nfinished_mutex = nullptr;
...
bool the_same_client_group(const std::string &a, const std::string &b)
{
    ...
    if (a[0] == 'd' || b[0] == 'd')
        return true;
    // Client hostnames are expected as cXY...; if malformed, don't match groups.
    if (a.size() < 2 || b.size() < 2)
        return false;
    return (a[0] == 'c' && b[0] == 'c' && a[1] == b[1]);
}
