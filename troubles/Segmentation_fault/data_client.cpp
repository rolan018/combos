...
int data_client_ask_for_files(ask_for_files_t params)
{
                    ...
                    if (uniform_int(0, 99, *g_rndg) < (int)project.ifgl_percentage)
                    {
                        ...
                        if (uniform_int(0, 99, *g_rndg) < (int)project.ifcd_percentage)
                        {
                            for (i = 0; i < workunit->ninput_files; i++)
                            {
                                ...
                                {
                                    // Expected data-client mailbox name format is "c<number>".
                                    // Defensive checks prevent out-of-bounds access in special runs.
                                    if (server_with_data.size() < 3 || !std::isdigit((unsigned char)server_with_data[1]))
                                    {
                                        fprintf(stderr,
                                                "[BOINC_DEBUG] data_client_ask_for_files: skip malformed replica name '%s'\n",
                                                server_with_data.c_str());
                                        fflush(stderr);
                                        continue;
                                    }
                                    int server_number = atoi(server_with_data.c_str() + 2) - g_total_number_ordinary_clients;
                                    if (server_number < 0 || server_number >= (int)SharedDatabase::_dclient_info.size())
                                    {
                                        fprintf(stderr,
                                                "[BOINC_DEBUG] data_client_ask_for_files: skip out-of-range replica '%s' -> idx=%d size=%zu\n",
                                                server_with_data.c_str(), server_number, SharedDatabase::_dclient_info.size());
                                        fflush(stderr);
                                        continue;
                                    }
    ...
    fprintf(stderr, "[BOINC_DEBUG] data_client_ask_for_files: pre nfinished_dclients t=%.6g host=%s proj=%d\n",
            sg4::Engine::get_clock(), dclient_info->server_name.c_str(), (int)project_number);
    fflush(stderr);
    {
        std::unique_lock lock(*SharedDatabase::dclient_nfinished_mutex);
        project.nfinished_dclients++;
    }
    fprintf(stderr, "[BOINC_DEBUG] data_client_ask_for_files: exit proj=%d\n", (int)project_number);
    fflush(stderr);
    return 0;
}
