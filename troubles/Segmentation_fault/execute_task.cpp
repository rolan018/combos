...
XBT_LOG_NEW_DEFAULT_CATEGORY(execute_task, "Logging channel for execute_task");
...
int client_execute_tasks(ProjectInstanceOnClient *proj)
{
    ...
    while (sg4::Engine::get_clock() < maxtt)
    {
        ...
        while (proj->tasks_ready.empty() && !proj->stop_requested.load())
        {
            proj->tasks_ready_cv_is_empty->wait(task_ready_lock);
        }
        if (proj->stop_requested.load() && proj->tasks_ready.empty())
        {
            break;
        }
        ...
        msg_task->start();
        /* Must follow start(): client_main_loop may interleave and call msg_task->suspend() only when safe.
         * If running==1 before start(), downtime can suspend while exec is INITED; SimGrid then defers kernel
         * suspend to Exec::do_start() after an instant finish → ActivityImpl::suspend on null model_action_. */
        task->running = 1;

        // if (err == MSG_OK)
        // Non-blocking completion loop (instead of msg_task->wait())
        ...
        while (sg4::Engine::get_clock() < maxtt)
        {
            ...
            if (proj->stop_requested.load())
            {
                break;
            }
            // Keep loop cooperative; 1 simulated second is usually enough
            sg4::this_actor::sleep_for(1.0);
        }
        // If interrupted/stop requested, do cleanup and continue
        if (!completed)
        {
        ...
        es::Switcher::switch_to(proj->client->name, es::State::Idle);
        task->time_spent_on_execution += sg4::Engine::get_clock() - task->last_start_time;
        auto duration_metric = g_measure_task_duration_per_project.find(proj->name);
        if (duration_metric != g_measure_task_duration_per_project.end() && duration_metric->second != nullptr)
            duration_metric->second->add_to_series(task->time_spent_on_execution);
...
static void schedule_job(client_t client, TaskT *job)
{
    ...
    if (job->running)
    {
        ...
        {
            // Проверяем, что suspend не вызывается для текущего потока
            if (client->running_project->thread && 
    ...
    {
        // Проверяем, что suspend не вызывается для текущего потока
        if (client->running_project->thread.get() != sg4::Actor::self())
        {
            client->running_project->thread->suspend();
        }
    }
...
static void client_cpu_scheduling(client_t client)
{
    ...
    while (1)
    {
        ...
        if (!great_debt_proj)
        {
            ...
            {
                // Используем первый проект для отправки уведомления
                client->projects.begin()->second->client->on = 1;
                client->projects.begin()->second->client->sched_cond->notify_all();
            }
            return;
        }
        ...
        if (task)
        {
            if (deadline_missed(task))
            {
                ...
                task->msg_task->cancel();
                free_task(task);
                task = nullptr;  // Сбрасываем указатель после освобождения
                continue;
            }
            // printf("Client (%s): Scheduling task(%s)(%p) of project(%s)\n", client->name, MSG_task_get_name(task->msg_task), task, task->project->name);
...
std::pair<int, int> client_main_loop(client_t client)
{
    ...
    while (ceil(sg4::Engine::get_clock()) < maxtt)
    {
        ...
        if (cl_state == ClientState::Unavailable)
        {
            ...
            /* Suspend only the execute_tasks actor (see execution_state.hpp). Do NOT call msg_task->suspend() from
             * this actor: it races start()/Exec::do_start() on the other actor and can SIGSEGV inside
             * ActivityImpl::suspend (null model_action_ / deferred suspend). Wall-time accounting below matches the
             * old intent without cross-actor Exec suspend. */
            if (client->running_project && client->running_project->thread)
            {
                if (client->running_project->thread.get() != sg4::Actor::self())
                {
