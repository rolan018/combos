...
/* Synchronization (std::mutex: never lock sg4::Mutex across join/comm; safe vs maestro/teardown) */
static std::mutex _oclient_mutex;
...
// Print client execution results
    ...
    {
        std::lock_guard<std::mutex> lock(_oclient_mutex);
        for (auto &[key, proj] : client->projects)
        {
