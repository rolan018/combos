...
FIXME: http://www.boinc-wiki.info/Work-Fetch_Policy */
        ...
        try
        {
            ...
            if (!selected_proj || !client->deadline_missed.empty() || work_percentage == 0)
            {
                ...
                if (were_waiting_on)
                {
                    /* Never sleep until a far-future testing tick only: cap so we retry shortfall / proj->on. */
                    const double cap = wait_now + static_cast<double>(project_testing_period);
                    const double wake = std::min(next_project_testing_period, cap);
                    client->work_fetch_cond->wait_until(lock, wake);
                }
                else
                {
                    /* Unbounded wait(lock) could idle the client for the whole sim if nothing notifies. */
